		...
		if not DoesEntityExist(dealer) then
			...
			if MissionStart == 0 and dist <= 2 then
				if Config.UseTarget then
					...
					if dist <= 1.5 then
						DrawText3D(Config.dealerCoords.x, Config.dealerCoords.y, Config.dealerCoords.z, Lang:t("mission.accept_mission"))
					...
					if GuardsDead == 1 and BlownUp == 0 then
						if Config.UseTarget then
							if BlowBackdoor == 0 and GuardsDead == 1 then
								exports.qbx_core:Notify(Lang:t("info.detonate_bomb_target"), "primary")
								BlowBackdoor = 1
							end
							...
							if BlowBackdoor == 0 then
								exports.qbx_core:Notify(Lang:t("info.detonate_bomb"), "primary")
								BlowBackdoor = 1
							end
		...
		if GuardsDead == 1 then
			if not IsEntityInWater(PlayerPedId()) then
				...
				if lib.progressBar({
					...
					exports.qbx_core:Notify(Lang:t('info.bomb_timer', {TimeToBlow = Config.TimeToBlow / 1000}), "error")
			...
			if Config.UseTarget then
				...
				if dist <= 4.5 then
					if PickupMoney == 0 then
						exports.qbx_core:Notify(Lang:t("info.take_money"), 'primary', 7500)
						PickupMoney = 1
					end
